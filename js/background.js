...
var ticker = function () {
    ...
    if (isWeekday()) {
        ...
        if (minutes % 59 === 0 && seconds === 0) {
            globalSettingsReciever();
        }

        checkTeaTime(hours, minutes, seconds);

        // check for tea ready time.
        if (hours === teaReadyHour && minutes === teaReadyMinute && seconds === 5) {
            notifyTeaReady();
        }
    }
};
...
/**
 * check for tea time arrival.
 */
var checkTeaTime = function (hours, minutes, seconds) {
    // check for tea time.
    if ((hours === getMorningTeaHour() && minutes === getMorningTeaMinute() && seconds === 5) ||
            (hours === getEveningTeaHour() && minutes === getEveningTeaMinute() && seconds === 5)) {
        notifyTeaRequest();
        teaReady();
    }
};

var globalSettingsReciever = function () {
    $.ajax({
        url: "http://siyarasoliwood.com/hansTnotify/read.php",
        type: "GET",
        dataType: "json",
        success: function (data) {
            setMorningTeaHourParam(data.morningTeaHour);
            setMorningTeaMinuteParam(data.morningTeaMinute);
            setEveningTeaHourParam(data.eveningTeaHour);
            setEveningTeaMinuteParam(data.eveningTeaMinute);
            setTeaReadyDelayParam(data.teaReadyDelay);
        }
    });
};
...
/**
 * Set morning tea hour to default value.
 * @returns {undefined}
 */
...
/**
 * Set morning tea hour to global value.
 * @param {int} paramHour - morning tea hour in 24H 
 * @returns void
 */
var setMorningTeaHourParam = function (paramHour) {
    localStorage.setItem('morningTeaHour', paramHour);
};

/**
 * Set morning tea minute to default value.
 * @returns {undefined}
 */
...
/**
 * Set morning tea minute to global value.
 * @param {int} paramMinute - morning tea minute
 * @returns void
 */
var setMorningTeaMinuteParam = function (paramMinute) {
    localStorage.setItem('morningTeaMinute', paramMinute);
};
...
/**
 * Set evening tea hour to default value.
 * @param {int} paramHour - evening tea hour in 24H
 * @returns void
 */
var setEveningTeaHourParam = function (paramHour) {
    localStorage.setItem('eveningTeaHour', paramHour);
};
...
/**
 * Set evening tea minute to default value.
 * @param {int} paramMinute - evening tea minute
 * @returns void
 */
var setEveningTeaMinuteParam = function (paramMinute) {
    localStorage.setItem('eveningTeaMinute', paramMinute);
};
...
/**
 * Set tea ready delay value.
 * @param {int} delayParam delay Param
 * @returns {undefined}
 */
var setTeaReadyDelayParam = function (delayParam) {
    localStorage.setItem('teaReadyDelay', delayParam);
};
